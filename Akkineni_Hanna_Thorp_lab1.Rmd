...
#TODO: For some reason Canada ends up in Caribbean after doing the below.  See if there are any others, Fix and note in Univariate analysis
...
# STILL WORKING ON THIS KH


```{r}
table_means = apply(correlation_matrix_input, 2, mean, na.rm = TRUE)
table_sums = apply(correlation_matrix_input, 2, sum, na.rm = TRUE)
table_sd = apply(correlation_matrix_input, 2, sd, na.rm = TRUE)
table_bind = cbind(table_means, table_sums, table_sd)
names(table_bind) = c("variable", "mean", "sum", "stddev")
table_output = table_bind[c("violations.pre", "violations.pos", "violations_weighted.cars_total.pre", "violations_weighted.cars_total.pos", "trade"), ]


kable(table_output, "latex", longtable = TRUE, booktabs = TRUE, caption = "Key Variable Summary Data Description") %>%
...
-----------------|------|---------|---------------
violations before enforcement change |`r mean(cor_oneline$violations.pre)`|`r sd(cor_oneline$violations.pre)`|`r sum(!is.na(cor_oneline$violations.pre))` 
violations after enforcement change |`r mean(cor_oneline$violations.pos)`|`r sd(cor_oneline$violations.pos)`|`r sum(!is.na(cor_oneline$violations.pos))` 
violations per car before enforcement change |`r mean(correlation_matrix_input$violations_weighted.cars_total.pre, na.rm = TRUE)`|`r sd(correlation_matrix_input$violations_weighted.cars_total.pre, na.rm = TRUE)`|`r sum(!is.na(correlation_matrix_input$violations_weighted.cars_total.pre))` 
violations per car after enforcement change |`r mean(correlation_matrix_input$violations_weighted.cars_total.pos, na.rm = TRUE)`|`r sd(correlation_matrix_input$violations_weighted.cars_total.pos, na.rm = TRUE)`|`r sum(!is.na(correlation_matrix_input$violations_weighted.cars_total.pos))` 
Cars per mission |`r mean(cor_oneline$cars_total, na.rm = TRUE)`|`r sd(cor_oneline$cars_total, na.rm = TRUE)`|`r sum(!is.na(cor_oneline$cars_total))` 
corruption       | ###  | ###     | # of non-NA's 
trade |`r round(table_means["cars_total"], 2)`|###|###` 
-->
=======
Variable         | Mean | Std Dev | Observations 
-----------------|------|---------|---------------
violations (log) | ###  | ###     | # of non-NA's 
corruption       | ###  | ###     | # of non-NA's 
>>>>>>> 65185487f468a78df1bd61641231376edd188f18
...
...
* the relationships between gov wage, gdp per capita and corruption
    + Countries with higher violations + higher corruption index might be low gov_wage_gdp or low GDP pc
    + GDP PC relates with trade, making the correlation between gdp and corruption and vilations a little merky still.
<<<<<<< HEAD
* ~~analyze gov_wage_gdp (particular after, when fines are being paid)~~ Correlation changes in step with the gerneral change after the enforcement
* ~~analyze trade relationship~~ (I will include in gdp and corruption)

During our exploritory data analysis, we revealed three relationships to corruptions that may confound our key relationship analysis.  

## A correlation between a corruption index and that country's Gross Domestic Product Per Capita
...
<<<<<<< HEAD
TODO: Describe unexplained change in correlation betwen total aid and violations/car  
