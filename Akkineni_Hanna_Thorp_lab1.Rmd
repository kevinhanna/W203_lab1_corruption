...
The objective of our analysis is to find if there is any relationship between corruption and parking tickets.  Of the variables above, both corruption and violations are plainly essential to our study in our Analysis of Key Relationships.  The number of violations alone is likely to mislead us as countries will have different number of staff and cars, during our Key Relationship Analysis, we'll explain why we're using cars and violations together.  Total aid provided by the US and the country's GDP per capita are also of interest and we'll explore those more in the Analysis of Secondary Effects. 
...
We don't fully understand the corruption variable other than to say it appears to be an index. The frequency distribution skews negative, however the median (`r round(median(correlation_matrix_input$corruption),2)`) is pretty close to 0 which suggests the values might be deliberately scaled to balance around 0, so we will not apply any transformations to corruption.  The outliers in the boxplot are expected with this skew.
...
Looking at the histogram on the left we see the vast majority of missions have 0 to 10 cars, which is unsuprising as are the ouliers seeing as some missions have a lot of staff.  We test this hypothesis by dividing the total number of cars by the the number of staff and spouses.  This is displayed in the boxplot on the right, there are still some outliers however, the maximum is 2.6, and that could be explained by the absence of children in our dataset and staff having more than one car.  The frequency is skewed negative, but this variable will be used to create an average with violations and we'll apply the transformation to that combination. 
...
The scale of the bins alone for the violations variable shows a significant change in behavior after the enforcement change. This is a sum of all violations for each mission and the number of cars that belong to each mission is different.  To better make sense of the effects corruption has on parking violations we need to create a varable that removes the effect cars has on violations.  
...
The frequency distribution of the transformation $violations/total cars$ above is skewed negatively giving cause to believe most diplomats are paying any parking fines in a timely maner.  This skewness however, will prevent our Analysis of Key Relationships yielding compelling results without some further transformation.
...
During our exploritory data analysis, we revealed some relationships to corruptions that may confound our key relationship analysis.  
...
The plot above shows the steep negative relationship of GDP per capita and corrution.  This is relevant to the key relationship between violations/total cars and corruption as GPD per capita might possitively corelate to the personal wealth of mission employees and wealth might also contribute to a person's decision to pay parking tickets.  

It is worth noting that the variable gov_wage_gdp which is the percentage of a country's GDP paid to government employees was analyzed by multiplying it to GDP per capita.  The correlation was weaker, however 57 of the 149 countries being analyzed are missing values for gov_wage_gdp, and we have no insights that would provide confidence that wages of mission employees changes in any predictable manner with gov_wage_gdp.
...
Total aid, which we assume is sum of an index (not dollars) of military and economic aid provided to the country by the U.S. is also correlated to corruption. In our analysis we noticed that the correlation between total aid and the violations changed from 0.44 to -0.06.  We investigated further and found there was another change in parking ticket payment enforcement made around the same time; unpaid parking could be deducted from aid provided to the country which may explain some of the change in the number violations at this time. Having made assumptions about the total aid variable we are only able to suggest their might be a confounding effect caused by aid.

## Percentage of Country Who Identify as Muslim
Our dataset included the percentage of a country's population that identified as Muslim, and that happens to correlate with the number of violations per car.  However, the percentage of Mulsims also correlates with GDP per capita, and GDP per capita has a much stronger correlation to the number of violations per car.  We don't feel any relationships to a single religion is valuable in absense of other major religions having different correlations with independent variables.    
...
* Total number of parking tickets dropped after regulation change.
* The counrties who had the most tickets before had the most after
* The slope of violations/cars & corrution flattened out after reg showing the fines had more impact on corrupt countries. 
